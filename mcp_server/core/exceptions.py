...
class InvalidInputError(ProtosMCPError):
    ...
    def __init__(self, parameter: str, issue: str, suggestion: Optional[str] = None):
        super().__init__(f"Invalid input for '{parameter}': {issue}", suggestion)
...
class PayloadTooLargeError(ProtosMCPError):
    """Raised when a context payload would exceed configured guardrails."""

    def __init__(self, size: int, limit: int, suggestion: Optional[str] = None):
        details = (
            f"Payload size {size:,} bytes exceeds the allowed limit of {limit:,} bytes"
        )
        hint = (
            suggestion
            or "Request a smaller preview or use download_entity to fetch data offline."
        )
        super().__init__(details, hint)
